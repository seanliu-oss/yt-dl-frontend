...
def download(url, audioOnly, checkSSL):
    '''
    ...
    try:
        ...
        pass
    if _down_path:
        ydl_opts['outtmpl']='/'.join([_down_path,ydl_opts['outtmpl']])
...
#        self.ids['downButton'].disabled=True
#        threading.Thread(target=download,args=(url,audioOnly,checkSSL)).start()
